...
parser.add_argument('-b', '--backup', required=False, type=str, default=False,
	help='Allows user to backup all save files\
		\nNOTE: to use include "-b" or "--backup" followed by the name of a backup folder you want to create in your command\
		\nEX: python Joi.py -b KDbkup |OR| python Joi.py --backup KDbkup')

parser.add_argument('-a', '--ai_reflect', required=False, action='store_true',
	help='Allows user to add an emotional reflection but the question is generated dynamically using chatgpt-3.5-turbo model\
		\nNOTE: to use just include "-a" or "--ai_reflect" in your command | NOT TO BE COMBINED WITH -r\
		\nEX: python Joi.py -a |OR| python Joi.py --ai_reflect')

arg = parser.parse_args()

# TODO organize the snot out of these optional arguments
...
if arg.backup:
	Joi_funxtions.create_backup(Joe, arg.backup)
	exit()
...
if arg.reflect:
	...
	if arg.ai_reflect:
		Diary+=Joi_funxtions.ai_reflect(Diary)
	else:
		try:
			Diary+=Joi_funxtions.reflect(Joe, Diary)
		except:
